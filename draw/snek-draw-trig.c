...
static const uint16_t _snek_draw_sin_table[1 << SNEK_DRAW_LOG2_SIN] = {
...
#if 0
#include <stdio.h>
#define snek_draw_angle_to_double(x) ((x) * 180.0 / SNEK_DRAW_ANGLE_180)
#define A(x) snek_draw_angle_to_double(x)
#define F(x) (x)
...
snek_draw_sin (snek_draw_angle_t a)
{
    ...
    /* limit to [0..360) */
    a = a & (SNEK_DRAW_ANGLE_360 - 1);
    /* special case for 90 degrees - no room in table */
    if ((a & ~(SNEK_DRAW_ANGLE_180)) == SNEK_DRAW_ANGLE_90)
	sin = SNEK_DRAW_FLOAT_ONE;
    else {
	/* mirror second and third quadrant values across y axis */
	if (a & SNEK_DRAW_ANGLE_90)
	    a = SNEK_DRAW_ANGLE_180 - a;
	sin = _snek_draw_sin_table[a & (SNEK_DRAW_ANGLE_90 - 1)] / 65536.0f;
    }
    /* mirror third and fourth quadrant values across x axis */
    if (a & SNEK_DRAW_ANGLE_180)
	sin = -sin;
...
{
    return snek_draw_sin (a + SNEK_DRAW_ANGLE_90);
}
...
snek_draw_tan (snek_draw_angle_t a)
{
    ...
    {
	if (s > 0)
	    return SNEK_DRAW_FLOAT_MAX;
	else
	    return SNEK_DRAW_FLOAT_MIN;
    }
    if (s == 0)
	return 0;
    return s / c;
